...
static const std::string PointHeaders{
    "point-id fixed posx posy velx vely mass color(rgba)"};
...
public:
  ...
  Sim(EntityManager &entities_, double gravity_ = 0)
      : entities(entities_), gravity(gravity_) {
    ...
    std::for_each(entities.springs.begin(), entities.springs.end(),
                  [&](Spring &spring) {
                    spring.springHandler(
                        entities.points[static_cast<std::size_t>(spring.p1)],
                        entities.points[static_cast<std::size_t>(spring.p2)]);
                  });

    // update point positions
    std::for_each(
        entities.points.begin(), entities.points.end(),
    ...
    for (const Polygon &poly : entities.polys) {
      for (Point &point : entities.points) {
        if (poly.isBounded(point.pos) &&
            poly.isContained(
                point.pos)) // not sure if bounded check is still faster
  ...
  std::pair<SpringId, double> findClosestSpring(const Vec2 pos) const {
    ...
    for (std::size_t i = 0; i != entities.springs.size(); ++i) {
      double dist = pos.distToLine(
          ...
          entities.points[static_cast<std::size_t>(entities.springs[i].p2)]
              .pos);
      if (dist < closestDist) {
        closestDist = dist;
  ...
  void load(std::filesystem::path path, bool replace, ObjectEnabled enabled,
    ...
    if (line != PointHeaders)
      throw std::runtime_error("Point headers invalid: \n is - " + line +
                               "\n should be - " + PointHeaders);

    std::stringstream ss;
    std::size_t index = 0;
    while (true) {
      std::getline(file, line);
      if (checkIfHeader(SpringHeaders, line))
        break;
      if (enabled.points) {
        Point point{};
    ...
    while (true) {
      std::getline(file, line);
      if (checkIfHeader(PolyHeaders, line))
        break;
      if (enabled.springs) {
        ss = std::stringstream(line);
        Spring spring{};
  ...
  static bool checkIfHeader(const std::string &header,
  ...
  static Sim softbody(EntityManager &entities, const Vector2<std::size_t> &size,
                      const Vec2 &simPos, float gravity, float gap,
                      float springConst, float dampFact,
    ...
    for (std::size_t x = 0; x != size.x; ++x) {
      for (std::size_t y = 0; y != size.y; ++y) {
        ...
        if (x < size.x - 1) {
          if (y < size.y - 1) {
            entities.addSpring(
                {springConst, dampFact,
                 std::numbers::sqrt2 * static_cast<double>(gap), p,
                 PointId{x + 1 + (y + 1) * size.x}}); // down right
          }
          entities.addSpring({springConst, dampFact, gap, p,
                              PointId{x + 1 + (y)*size.x}}); // right
        }
        if (y < size.y - 1) {
          if (x > 0) {
            entities.addSpring(
